    ...
    for current_event in data.iter().rev() {
        match current_event.timestamp {
            ...
            Timestamp::Interval { .. } => {
                ...
                let thread_stack = threads.entry(current_event.thread_id).or_default();
                ...
                while let Some(current_top) = thread_stack.last().cloned() {
                    ...
                    let _popped = thread_stack.pop().unwrap();
                }

                // If there is something on the stack, subtract the current
                // interval from it.
                if let Some(current_top) = thread_stack.last() {
                ...
                // Update the total_time counter
                let current_event_is_top_level = thread_stack.is_empty();
                if current_event_is_top_level {
                    total_time += current_event.duration().unwrap();
                }
